...
public class CustomerService {
    ...
    public Customer save(Customer customer) {
        if (customer == null) {
            throw new CustomerSaveException("Customer can't be null");
        }
        if (customer.getName() == null || customer.getName().trim().isEmpty()) {
            throw new CustomerSaveException("Customer name can't be null or empty");
        }
    ...
    public void update(Long id, String newName) {
        if (newName == null || newName.trim().isEmpty()) {
            throw new CustomerUpdateException("Customer name can't be null or empty");
        }
    ...
    public void deleteByName(Long id, String name) {
        getAllActiveCustomers()
                ...
                .forEach(customer -> customer.setActive(false));
    }
    ...
    public void removeProductFromCustomersCart(Long customerId, Long productId) {
        // Подход 1. Удаление всех продуктов одного названия из корзины
//        Customer customer = getActiveCustomerById(customerId);
//        customer.getCart().removeIf(product -> product.getId().equals(productId));

        // Подход 2. Удаление только одного продукта нужного наименования из корзины
        Customer customer = getActiveCustomerById(customerId);
        Product product = productService.getActiveProductById(productId);
